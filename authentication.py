...
RAJK_ZORK_edges_from_AWS_server = client.get_object(
    Bucket=szovegek, Key=edges
)
RAJK_ZORK_nodes_from_AWS_server = client.get_object(
    Bucket=szovegek, Key=nodes
)
edge_data = pd.read_csv(RAJK_ZORK_edges_from_AWS_server["Body"])\
    .set_index("FROM")
node_data = pd.read_csv(RAJK_ZORK_nodes_from_AWS_server["Body"])\
    .set_index("NODE_ID")
